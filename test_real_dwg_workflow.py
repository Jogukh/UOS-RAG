...
def test_dwg_conversion():
    ...
    try:
        ...
        # 기존에 변환된 DXF 파일 확인
        project_path = Path("uploads") / project_name
        converted_dxf_dir = project_path / "converted_dxf"
        
        if converted_dxf_dir.exists():
            existing_dxf_files = list(converted_dxf_dir.rglob("*.dxf"))
            
            if existing_dxf_files:
                print(f"✅ 기존 변환된 DXF 파일 발견: {len(existing_dxf_files)}개")
                
                # 기존 파일들을 변환 결과로 반환
                conversion_results = {}
                for dxf_file in existing_dxf_files:
                    # 해당하는 DWG 파일 경로 추정
                    relative_path = dxf_file.relative_to(converted_dxf_dir)
                    dwg_path = project_path / relative_path.with_suffix('.dwg')
                    conversion_results[str(dwg_path)] = str(dxf_file)
                
                print(f"📋 변환 결과:")
                for i, (dwg_path, dxf_path) in enumerate(list(conversion_results.items())[:10], 1):
                    dwg_name = Path(dwg_path).name
                    dxf_name = Path(dxf_path).name
                    print(f"  {i:2d}. {dwg_name} -> {dxf_name}")
                
                if len(conversion_results) > 10:
                    print(f"  ... 및 {len(conversion_results) - 10}개 추가 파일")
...
def test_full_workflow():
    ...
    try:
        ...
        # 워크플로우 실행
        print("\n🔄 워크플로우 실행 중...")
        result = workflow.run_workflow(project_path, "dwg_only")
